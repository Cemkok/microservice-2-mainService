...
public class JwtProvider  implements IJwtProvider{
	 ...
	 * Hence, below is the tool to generate RSA key online. It generates RSA public
	 * key as well as the private key of size 512 bit, 1024 bit, 2048 bit, 3072 bit
	 * and 4096 bit with Base64 encoded. By default, the private key is generated in
	 * PKCS#8 format and the public key is generated in X.509 format.
	 */
	...
	public Authentication getAuhentication(HttpServletRequest request) {
		...
		List<GrantedAuthority> authorities = Arrays.stream(claims.get("roles").toString().split(","))
				.map(role -> role.startsWith("ROLE_") ? role : "ROLE" + role).map(SimpleGrantedAuthority::new)
